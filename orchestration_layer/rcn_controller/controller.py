...
def get_rcn_info(rcn):
    ...
    offload_policy = spec["offloadPolicy"]
...
def build_env_vars(rcn):
    ...
    env_vars = [
        ...
        client.V1EnvVar(name="OFFLOAD_POLICY", value=offload_policy),
    ...
    return {
        "spec": {
            "template": {
                "spec": {
                    "containers": [
                        {"name": "pod-controller-container", "env": env_vars}
                    ]
                }
            }
        }
...
def patch_pod_controller(rcn):
    ...
    deployment_name = f"{rcn["metadata"]["name"]}-pod-controller"
